...
def get_sift_feature(img):
    ...
    d_sift.setDouble('initFeatureScale', SCALE)
    d_sift.setInt('initXyStep', STEP)
...
def linear_r(sc_train, sc_test):
    ...
    labels = clf.predict(x_test)
    accuracy = float(np.sum(labels == y_test)) / n_test
    logging.info('logistic regression Accuracy is ' + str(accuracy))
...
def get_dictionary():
    ...
    kmeans = MiniBatchKMeans(n_clusters=CLUSTER_K, batch_size=500).fit(sift_features_all)
...
def classify():
    ...
    logging.info('Cluster is ' + str(CLUSTER_K) + ', scale is ' + str(SCALE) + ', step is ' + str(STEP))
    knn(sc_train, sc_test)
    linear_r(sc_train, sc_test)

def run():
    logging.basicConfig(filename='hw2.log', filemode="a+", level=logging.DEBUG)
